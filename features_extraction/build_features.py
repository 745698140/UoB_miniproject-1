...
def cumulative_sum_price_levels(ask_limit_orders:list, bid_limit_orders:list,k:int)->Tuple[float, float]:
    ask_levels = len(ask_limit_orders)
    bid_levels = len(bid_limit_orders)
    common_levels = min(ask_levels, bid_levels)
    if k > common_levels:
        print(f"{k} is out of bounds, we replace it with {common_levels}")
        k = common_levels
    return sum(ask_limit_orders[:common_levels]), sum(ask_limit_orders[:common_levels])
